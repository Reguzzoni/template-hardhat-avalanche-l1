...
async function main() {
    ...
    console.log("Balance issuer", await commercialPaperContract.balanceOf(issuer));

    // check if the issuer has enough token
    const tokenAmount = 1;
    if ((await commercialPaperContract.balanceOf(issuer)) < tokenAmount) {
    ...
    let commercialPaperIsLive = await commercialPaperContract.status();
    console.log("commercialPaperIsLive", commercialPaperIsLive);
    if (commercialPaperIsLive != 1) {
        console.log("Commercial Paper is not Live, cannot transfer token to HLC");
        return;
    }
    ...
    await commercialPaperContract.connect(issuer).approve(hlcAddress, 1);
